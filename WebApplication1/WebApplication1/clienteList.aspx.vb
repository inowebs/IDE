...
Public Class WebForm9
    ...
    Protected Sub ingresar_Click(sender As Object, e As EventArgs) Handles ingresar.Click
        myCommand = New SqlCommand("SELECT id FROM admin WHERE nombre='" + pass1.Text.Trim + "'")
    ...
    Private Function bajarAcuse(ByVal mensAn, ByVal idCliente, ByVal idDecla, ByVal ejercicio, ByVal normalComplementaria, ByVal mes) As String
        ...
        Using dr5 = ExecuteReaderFunction(myCommand)
            dr5.Read()
    ...
    Protected Sub verDecl_Click(sender As Object, e As EventArgs) Handles verDecl.Click
        ...
        Using dr = ExecuteReaderFunction(myCommand)
            While dr.Read()
                ...
                myCommand4 = New SqlCommand(q4)
                Using dr4 = ExecuteReaderFunction(myCommand4)
                    If dr4.Read() Then
                        ...
                        Else
                            ...
                            myCommand = New SqlCommand(q)
                            Using dr8 = ExecuteReaderFunction(myCommand)
                                If dr8.HasRows Then
                                    While dr8.Read()
                                        ...
                                        myCommand2 = New SqlCommand(q2)
                                        Using dr6 = ExecuteReaderFunction(myCommand2)
                                            While dr6.Read()
...
etqContratos:
                            ...
                            myCommand2 = New SqlCommand(q2)
                            Using dr7 = ExecuteReaderFunction(myCommand2)
                                If dr7.HasRows Then
                                    While dr7.Read()
    ...
    Private Sub GridView3_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GridView3.RowCommand
        If e.CommandName = "logearse" Then
            Dim rowIndex As Integer = Convert.ToInt32(e.CommandArgument)
            Dim row As GridViewRow = GridView3.Rows(rowIndex)
            idCli.Text = row.Cells(1).Text
            Response.Write("<script language='javascript'>window.open('Login.aspx?id=" + idCli.Text + "&lan=1','_blank');</script>")
            'Response.Redirect("Login.aspx?id=" + idCli.Text)
        End If
    End Sub
    ...
    Protected Sub GridView3_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GridView3.SelectedIndexChanged
        ...
        myCommand = New SqlCommand(q)
        Dim v = ExecuteScalarFunction(myCommand)
        edoCli.SelectedValue = v

        tv22.Nodes.Clear()
        v = returnID("estatusDecla2", "pendiente recibir archivos").ToString
        Dim q2 = "SELECT id, mes, ejercicio, idEstatusDecla,idContrato FROM ideMens2 WHERE idEstatusDecla<>" + v + " and idCliente IN (SELECT id FROM clientes WHERE correo='" + correo.Text + "') order by ejercicio,cast(mes as int)"
        myCommand2 = New SqlCommand(q2)
        Using dr3 = ExecuteReaderFunction(myCommand2)
            While dr3.Read()
                Dim mes As New TreeNode()
        ...
        tvDeclas.Nodes.Clear()
        q = "SELECT ideAn.id,ideAn.ejercicio, ideAn.estado,ideAn.normalComplementaria, ideAn.idContrato, cli.correo, ideAn.fechaEnvio FROM ideAnual ideAn, clientes cli WHERE ideAn.idCliente=cli.id AND cli.correo='" + correo.Text + "' order by cast(ejercicio as int) DESC,id DESC"
        myCommand = New SqlCommand(q)
        Using dr = ExecuteReaderFunction(myCommand)
            While dr.Read()
                Dim ejercicio As New TreeNode()
                ejercicio.Text = dr("correo") + "," + dr("ejercicio") + ", Estatus " + dr("estado").ToString.ToLower + ", Id " + dr("id").ToString + ", Tipo " + dr("normalComplementaria").ToString.ToLower + ", fechaEnvio " + dr("fechaEnvio").ToString
                If dr("estado").ToString <> "VACIA" Then
                    ejercicio.Text = ejercicio.Text + ", Contrato " + dr("idContrato").ToString
                End If
                tvDeclas.Nodes.Add(ejercicio)
        ...
        Dim q3
        q3 = "SELECT ideM.id, ideM.mes, ideM.estado,ideM.normalComplementaria, ideM.idContrato, ideM.fechaEnvio, cli.correo, ideAn.ejercicio FROM ideMens ideM, ideAnual ideAn, clientes cli WHERE ideAn.idCliente=cli.id AND ideM.idAnual=ideAn.id AND ideM.estado<>'VACIA'  AND cli.correo='" + correo.Text + "' order by ideAn.ejercicio, cast(ideM.mes as int),ideM.id DESC"
        myCommand2 = New SqlCommand(q3)
        Using dr3 = ExecuteReaderFunction(myCommand2)
            While dr3.Read()
                Dim mes As New TreeNode()
                mes.Text = dr3("correo") + ", Año " + dr3("ejercicio") + ", Mes " + dr3("mes") + ", Estatus " + dr3("estado").ToString.ToLower + ", Id " + dr3("id").ToString + ", Tipo " + dr3("normalComplementaria").ToString.ToLower + ", Contrato " + dr3("idContrato").ToString + ", fechaEnvio " + dr3("fechaEnvio").ToString
                TreeView2.Nodes.Add(mes)
            End While
        End Using

        SqlDataSourceContras.SelectCommand = "SELECT co.id,co.precionetocontrato,co.fechaPago,co.periodoInicial,pla.elplan,co.fechaFinal,co.nDeclHechas,co.nDeclContratadas,co.uuid FROM contratos co, clientes cli, planes pla WHERE co.idCliente=cli.id AND co.idPlan=pla.id AND cli.correo='" + correo.Text + "' ORDER BY co.id DESC"
        GridViewContras.DataBind()
    ...
    Private Sub delCta_Click(sender As Object, e As EventArgs) Handles delCta.Click
        myCommand = New SqlCommand("DELETE From ideMens Where idAnual IN (SELECT ID FROM IDEANUAL WHERE idCliente=" + idCli.Text + ")")
        ExecuteNonQueryFunction(myCommand)
        myCommand = New SqlCommand("DELETE From ideAnual Where idCliente=" + idCli.Text)
        ExecuteNonQueryFunction(myCommand)
        myCommand = New SqlCommand("DELETE From reprLegal Where idCliente=" + idCli.Text)
        ExecuteNonQueryFunction(myCommand)
        myCommand = New SqlCommand("DELETE From contratos Where idCliente=" + idCli.Text)
        ExecuteNonQueryFunction(myCommand)
        myCommand = New SqlCommand("DELETE From clientes Where id=" + idCli.Text)
        ExecuteNonQueryFunction(myCommand)
    ...
    Protected Sub declaBusPer_Click(sender As Object, e As EventArgs) Handles declaBusPer.Click
        If declaDesde.Text.Trim <> "" Then
            Dim dtnow As DateTime
            Dim regDate As New System.Text.RegularExpressions.Regex("^\d{1,2}\/\d{1,2}\/\d{4}$")
            If regDate.IsMatch(declaDesde.Text.Trim) Then
                If Not DateTime.TryParse(declaDesde.Text.Trim, dtnow) Then
                    declaDesde.Focus()
                    Response.Write("<script language='javascript'>alert('fecha inicial invalida');</script>")
                    Exit Sub
                End If
            Else
                declaDesde.Focus()
                Response.Write("<script language='javascript'>alert('fecha inicial  formato no valido (dd/mm/aaaa)');</script>")
                Exit Sub
            End If
        End If

        If declaHasta.Text.Trim <> "" Then
            Dim dtnow As DateTime
            Dim regDate As New System.Text.RegularExpressions.Regex("^\d{1,2}\/\d{1,2}\/\d{4}$")
            If regDate.IsMatch(declaHasta.Text.Trim) Then
                If Not DateTime.TryParse(declaHasta.Text.Trim, dtnow) Then
                    declaHasta.Focus()
                    Response.Write("<script language='javascript'>alert('fecha inicial invalida');</script>")
                    Exit Sub
                End If
        ...
        If declaDesde.Text > declaHasta.Text Then
            Response.Write("<script language='javascript'>alert('fecha inicial  debe ser menor a la fecha final');</script>")
            Exit Sub
        End If

        tv22.Nodes.Clear()
        Dim v = returnID("estatusDecla2", "pendiente recibir archivos").ToString
        Dim q2 = "SELECT id, mes, ejercicio, idEstatusDecla,idContrato FROM ideMens2 WHERE idEstatusDecla<>" + v + " and fPresentada>='" + Format(Convert.ToDateTime(declaDesde.Text.Trim), "yyyy-MM-dd") + "' AND fPresentada<='" + Format(Convert.ToDateTime(declaHasta.Text.Trim), "yyyy-MM-dd") + "' order by ejercicio,cast(mes as int)"
        myCommand2 = New SqlCommand(q2)
        Using dr3 = ExecuteReaderFunction(myCommand2)
            While dr3.Read()
                Dim mes As New TreeNode()
        ...
        tvDeclas.Nodes.Clear()
        Dim q = "SELECT ideAn.id,ideAn.ejercicio, ideAn.estado,ideAn.normalComplementaria, ideAn.idContrato, cli.correo, ideAn.fechaEnvio FROM ideAnual ideAn, clientes cli WHERE ideAn.idCliente=cli.id AND ideAn.fechaEnvio>='" + Format(Convert.ToDateTime(declaDesde.Text.Trim), "yyyy-MM-dd") + "' AND ideAn.fechaEnvio<='" + Format(Convert.ToDateTime(declaHasta.Text.Trim), "yyyy-MM-dd") + "' order by cast(ejercicio as int) DESC,id DESC"
        myCommand = New SqlCommand(q)
        Using dr = ExecuteReaderFunction(myCommand)
            While dr.Read()
                Dim ejercicio As New TreeNode()
                ejercicio.Text = dr("correo") + ", " + dr("ejercicio").ToString + ", Estatus " + dr("estado").ToString.ToLower + ", Id " + dr("id").ToString + ", Tipo " + dr("normalComplementaria").ToString.ToLower + ", FechaEnvio " + dr("fechaEnvio").ToString
                If dr("estado").ToString <> "VACIA" Then
                    ejercicio.Text = ejercicio.Text + ", Contrato " + dr("idContrato").ToString
                End If
                tvDeclas.Nodes.Add(ejercicio)
                ...
                q2 = "SELECT ideM.id, ideM.mes, ideM.estado,ideM.normalComplementaria, ideM.idContrato FROM ideMens ideM, ideAnual ideAn WHERE ideAn.id=" + dr("id").ToString + " AND ideM.idAnual=ideAn.id AND ideM.estado<>'VACIA'  AND ideM.fechaEnvio>='" + Format(Convert.ToDateTime(declaDesde.Text.Trim), "yyyy-MM-dd") + "' AND ideM.fechaEnvio<='" + Format(Convert.ToDateTime(declaHasta.Text.Trim), "yyyy-MM-dd") + "' order by cast(ideM.mes as int),ideM.id"
                myCommand2 = New SqlCommand(q2)
                Using dr2 = ExecuteReaderFunction(myCommand2)
                    While dr2.Read()
                        Dim mes As New TreeNode()
        ...
        TreeView2.Nodes.Clear()
        Dim q3
        q3 = "SELECT ideM.id, ideM.mes, ideM.estado,ideM.normalComplementaria, ideM.idContrato, ideM.fechaEnvio, cli.correo, ideAn.ejercicio FROM ideMens ideM, ideAnual ideAn, clientes cli WHERE ideAn.idCliente=cli.id AND ideM.idAnual=ideAn.id AND ideM.estado<>'VACIA'  AND ideM.fechaEnvio>='" + Format(Convert.ToDateTime(declaDesde.Text.Trim), "yyyy-MM-dd") + "' AND ideM.fechaEnvio<='" + Format(Convert.ToDateTime(declaHasta.Text.Trim), "yyyy-MM-dd") + "' order by ideAn.ejercicio, cast(ideM.mes as int),ideM.id  DESC"
        myCommand2 = New SqlCommand(q3)
        Using dr3 = ExecuteReaderFunction(myCommand2)
            While dr3.Read()
                Dim mes As New TreeNode()
        ...
        SqlDataSourceContras.SelectCommand = "SELECT co.id,co.precionetocontrato,co.fechaPago,co.periodoInicial,pla.elplan,co.fechaFinal,co.nDeclHechas,co.nDeclContratadas,co.uuid FROM contratos co, clientes cli, planes pla WHERE co.idCliente=cli.id AND co.idPlan=pla.id AND co.fecha>='" + Format(Convert.ToDateTime(declaDesde.Text.Trim), "yyyy-MM-dd") + "' AND co.fecha<='" + Format(Convert.ToDateTime(declaHasta.Text.Trim), "yyyy-MM-dd") + "' ORDER BY co.id DESC"
        GridViewContras.DataBind()
